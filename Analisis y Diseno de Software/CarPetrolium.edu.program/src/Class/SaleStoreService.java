...
public class SaleStoreService {
    ...
    public SaleStoreService(Customer cliente, Employee empleado, double total) {
        if(cliente == null)
    ...
    public void sale(){
        JOptionPane.showMessageDialog(null,"REALIZADO POR: " + this.empleado.getNombre() + "\nCLIENTE: "+ this.cliente.getNombre() +"\nTOTAL A PAGAR: "+ this.total
                          +" DOLARES","**INFO VENTA MINIMERCADO**",JOptionPane.INFORMATION_MESSAGE);
    }

    @Override
    public String toString() {
        return "CLIENTE" + cliente.getNombre() + ", EMPLEADO: " + empleado.getNombre() + ", TOTAL: " + total + " EN PRODUCTOS";
    }
